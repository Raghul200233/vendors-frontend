...
import { 
  ...
  AlertCircle, CheckCircle, Edit2, Save, Map, Camera
} from 'lucide-react';
...
const VendorProfile = () => {
  ...
  const handleImageUpload = async (type, files) => {
    ...
    try {
      const response = await axios.post(`${API_URL}/vendor/upload-${type}`, formData, {
        headers: { 
      ...
      setProfile(prev => ({ ...prev, [type === 'logo' ? 'logo' : 'banner']: response.data.data }));
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      {/* Banner Section */}
      <div className="relative h-48 md:h-64 rounded-xl overflow-hidden mb-16 bg-gradient-to-r from-blue-600 to-purple-600">
        {profile?.banner ? (
          <img src={profile.banner} alt="Banner" className="w-full h-full object-cover" />
        ) : (
          <div className="w-full h-full flex items-center justify-center">
            <Store className="w-20 h-20 text-white opacity-30" />
          </div>
        )}
        {editing && (
          <label className="absolute bottom-4 right-4 bg-black bg-opacity-50 text-white px-3 py-2 rounded-lg cursor-pointer hover:bg-opacity-70">
        ...
        <div className="absolute -bottom-12 left-8">
          <div className="relative">
            <div className="w-24 h-24 bg-white rounded-full shadow-lg flex items-center justify-center overflow-hidden border-4 border-white">
              {profile?.logo ? (
                <img src={profile.logo} alt={profile.storeName} className="w-full h-full object-cover" />
              ) : (
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1">
                  ...
                  <span className="text-xs text-gray-500 ml-2">
                    (Share your shop location from Google Maps)
                  </span>
                </label>
                {editing ? (
                  <input
                    ...
                    className="w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="https://maps.google.com/?q=Your+Shop+Location"
                  />
                ) : (
                  profile?.location?.googleMapLink ? (
                    <a 
                      href={profile.location.googleMapLink} 
                      target="_blank" 
                    ...
                    <p className="text-gray-500">No location added</p>
                  )
                )}
              </div>
              ...
              {profile?.location?.googleMapLink && (
                <div className="mt-4 border rounded-lg overflow-hidden">
                  <iframe
                    src={extractGoogleMapEmbed(profile.location.googleMapLink)}
                    width="100%"
                    height="300"
                    style={{ border: 0 }}
                    allowFullScreen
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
              ...
              {editing && (
                <label className="border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center h-32 cursor-pointer hover:border-blue-500">
              ...
              <div>
                ...
                {editing ? (
                  ...
                  profile?.socialLinks?.instagram ? (
                    <a href={profile.socialLinks.instagram} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                      {profile.socialLinks.instagram}
                    </a>
                  ) : <p className="text-gray-500">Not added</p>
                )}
              ...
              <div>
                ...
                {editing ? (
                  ...
                  profile?.socialLinks?.whatsapp ? (
                    <a href={profile.socialLinks.whatsapp} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                      {profile.socialLinks.whatsapp}
                    </a>
                  ) : <p className="text-gray-500">Not added</p>
                )}
